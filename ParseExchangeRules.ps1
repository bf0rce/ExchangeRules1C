...
function ЗаписатьКлючевыеСвойстваПравилОбмена() {
	...
	foreach ($Элемент in $ДочерниеЭлементы) {
		...
		if ( ЭлементСодержитПрограммныйКод($Элемент) ) {
			Write-Verbose "Извлечение программного кода для элемента '$($Элемент.nodeName)'"
...
function ЗаписатьПрограммныйКод($Элемент, $ИсходныйКаталог) {
	...
	[void](New-Item -ItemType File -Force -Path $Файл)
	
	Write-Verbose "Запись программного кода в файл '$Файл'"
	$Элемент.text | Out-File -FilePath $Файл -Encoding utf8
	$Элемент.text = ""
...
try {
	...
	Write-Warning "Исходный XML-файл не загружен: $ФайлПравилОбмена"
	Write-Warning $_
	exit
}
