...
.EXAMPLE
.\CollectExchangeRules.ps1 -Verbose
...
function ДобавитьПрограммныйКод($Элемент, $ИсходныйКаталог) {
	...
	Write-Verbose "Чтение программного кода из файла '$ИмяФайла'"
	$СодержимоеФайла = Get-Content -Raw -Path $ИмяФайла	
...
function ПрочитатьЭлементИзФайла($ИмяФайла) {
	...
	Write-Verbose "Чтение xml-файла '$ИмяФайла'"
	try {
		...
		if ( $ЭлементыКоллекции.parseError.errorCode -ne 0 ) { throw $ЭлементыКоллекции.parseError.reason }
	} catch {
		Write-Warning "Файл не загружен: $ИмяФайла"
		Write-Warning $_
		exit
	}
